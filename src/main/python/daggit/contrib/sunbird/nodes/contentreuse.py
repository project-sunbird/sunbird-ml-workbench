...
from daggit.contrib.sunbird.oplib.contentreuseUtils import create_graph, create_node_relationships
...
class CreateDTBMapping(BaseOperator):
    @property
    def inputs(self):
        """
        Function that the ContentReuseEvaluation operator defines while returning graph inputs
...
class WriteRelationshipsToNeo4j(BaseOperator):
    ...
    def run(self):
        ...
        try:
            ...
            graph = create_graph(scheme, host, port, user, password, max_connections, secure)
